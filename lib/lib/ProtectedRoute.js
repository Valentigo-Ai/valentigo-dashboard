...
export default function ProtectedRoute({ children }) {
  ...
  useEffect(() => {
    async function checkUser() {
      ...
      if (!session) {
        ...
        // add a short fade-out animation before revealing content
  ...
  if (loading) {
    return (
      <div
        className={`flex flex-col items-center justify-center min-h-screen bg-[#0e1013] text-gray-300 transition-opacity duration-700 ${
